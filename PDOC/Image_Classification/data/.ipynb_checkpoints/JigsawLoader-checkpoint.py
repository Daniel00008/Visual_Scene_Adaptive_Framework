...
class JigsawNewDataset(data.Dataset):
    def __init__(self, args, names, labels, jig_classes=100, img_transformer=None, tile_transformer=None, patches=True,
        ...
        self.task = args.task
        if self.task == 'PACS':
            self.data_path = "./dataset/PACS"
        elif self.task == 'VLCS':
            self.data_path = "./dataset"
        elif self.task == 'HOME':
            self.data_path = "./dataset"
        elif self.task == 'DNET':
            self.data_path = "./dataset/domain_net"
        else:
    ...
    def __getitem__(self, index):
        framename = self.data_path + '/' + self.names[index]
        img = Image.open(framename).convert('RGB')
        if self.task == 'PACS' or self.task == 'VLCS':
            return self._image_transformer(img), 0, int(self.labels[index] - 1)
        elif self.task == 'HOME' or self.task =='DNET':
            return self._image_transformer(img), 0, int(self.labels[index])
